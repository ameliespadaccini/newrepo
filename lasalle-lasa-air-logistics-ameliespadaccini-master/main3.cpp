...
void set(int index, const Airport& value){
      int position = 0;
      Node* temp = head;
      while (temp->next != nullptr && position < index) {
          temp = temp->next;
...
int main()
{
    ...
    for (int i=0; i<airportCount; i++)
    {
      ...
      //cout << myList.get(i).latitude << " " << myList.get(i).longitude << "\n";
      cout << myList.get(i).code << myList.get(i).latitude << " " << myList.get(i).longitude << '\n';
      double dist = distanceEarth(ausLat, ausLong, lat1, long1);
      if (dist <= 160.934){ //100 miles = 160 km
        cout << myList.get(i).code << "," <<  myList.get(i).latitude << " " << myList.get(i).longitude << "\n";
      }
    }
    ...
    for (int i=0; i<airportCount; i++)
    {
      ...
      if (dist>farthest){
        farthest = dist;
        farthestPort = myList.get(i).code;
      }
    }


    cout << "Farthest airport:" << farthestPort;
